...
//      return traitsObj.operator()( keyVal ) % maxidx;
   ...
   size_type bucket_size( size_type n ) const
   {
      ...
      size_t count = 0;
      iterator it1 = myVec.myData.myFirst[2 * n];
   ...
   void erase( const key_type &keyVal )
   {
      ...
      {
          size_t pos = bucket(keyVal);
          if (myVec.myData.myFirst[2 * pos] == myVec.myData.myFirst[2 * pos + 1]) {
              myVec.myData.myFirst[2 * pos] = myList.end();
              myVec.myData.myFirst[2 * pos + 1] = myList.end();
          }
          else if (it == myVec.myData.myFirst[2 * pos])
              myVec.myData.myFirst[2 * pos]++;
          else if (it == myVec.myData.myFirst[2 * pos + 1])
              myVec.myData.myFirst[2 * pos + 1]--;
...
private:
   ...
   {
       iterator* it = &myVec.myData.myFirst[2 * bucket(val)];
       iterator* it1 = it + 1;
       (*it) = myList.insert(*it, val);

       if (*it1 == myList.end()) // when the bucket_size of val only one
           *it1 = *it;
   }
