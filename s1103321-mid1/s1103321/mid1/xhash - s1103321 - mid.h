...
private:
   ...
   {
       size_t pos = bucket(val);
       myVec.myData.myFirst[2 * pos] = myList.insert(myVec.myData.myFirst[2 * pos], val);
       if (myVec.myData.myFirst[2 * pos + 1] == myList.end())
           myVec.myData.myFirst[2 * pos + 1] = myVec.myData.myFirst[2 * pos];
   }
